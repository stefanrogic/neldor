...
export function ReadingContainer({ paragraph }: { paragraph: string[] }) {
  ...
  const buttonDefaultStyle = "bg-[#ffffff] p-5 text-[#000000] font-bold transition duration-100 ease-in-out hover:bg-[#adadad] hover:text-[#ffffff]";
    ...
    <>
      <nav className="fixed bottom-0 z-50 flex w-full flex-row justify-between p-[50px] opacity-50 transition duration-150 ease-in-out hover:opacity-100">
        <div className="flex flex-row gap-2">
          <AnimatePresence>
            <motion.span key={0} className="me-[25px] mt-auto text-wrap text-center" variants={variants} initial="hide" animate="show" exit="hide">
              {pause ? `Default WPS: ${defaultSpeed}ms` : `Current WPS: ${currentSpeed}ms`}
            </motion.span>
          </AnimatePresence>
          ...
          <AnimatePresence>
            {pause ? (
              <>
                <motion.button key={1} className={buttonDefaultStyle} variants={variants} initial="hide" animate="show" exit="hide" onClick={() => setDefaultSpeed(defaultSpeed > 0 ? defaultSpeed - 100 : 0)}>
                ...
                <motion.button key={2} className={buttonDefaultStyle} variants={variants} initial="hide" animate="show" exit="hide" onClick={() => setDefaultSpeed(defaultSpeed + 100)}>
                  <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
        ...
        <div className="mt-auto flex flex-row gap-2">
          <AnimatePresence>
            ...
            <motion.button key={4} className={buttonDefaultStyle} variants={variants} initial="hide" animate="show" exit="hide" onClick={() => setPause(!pause)}>
              <AnimatePresence>
                {pause ? (
                  <motion.svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" variants={variants} initial="hide" animate="show" exit="hide">
                    <path
      ...
      <div className="flex h-[100vh] flex-row items-center justify-center gap-20 p-[50px]">
        <AnimatePresence>
          {pause ? (
            <motion.div className="flex max-h-[80%] w-1/3 flex-col items-start justify-start bg-white text-black" initial={{ width: 0 }} animate={{ width: "33%" }} exit={{ width: 0 }}>
              <h2 className="mx-auto my-10 text-center text-2xl font-bold">Mali Princ - Odlomak</h2>
              <motion.p key={5} className="mx-10 mb-20 hidden xl:block" variants={variants} initial="hide" animate="show" exit="hide">
                {paragraph.map((p, i) => (
                  <>
                    <span key={i} className={i === wordIndex - 1 ? "bg-slate-500" : ""}>
                      {p + " "}
                    </span>
                  </>
                ))}
        ...
        <div className="flex w-2/3 flex-col items-center justify-center gap-2">
          ...
          <AnimatePresence>
            {pause ? (
              <motion.span key={6} className="text-center text-xl font-light text-[#bdbdbd]" variants={variants} initial="hide" animate="show" exit="hide">
                Mali Princ - Odlomak
              </motion.span>
            ) : null}
          </AnimatePresence>
